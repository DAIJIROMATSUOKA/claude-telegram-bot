...
### Nightly Forge v2 Chrome (2026-03-15 G11+G12+G13)
- **Script:** scripts/nightly-forge-chrome.sh
- **方式:** Chrome Worker Tab (1:6) + ローカルexecループ
- **G12:** 開始時にDESIGN-RULES.md inject+ドレイン
- **G13:** 各ステップ5行チェックポイント（log + Obsidian NightlyForge/ + chat投入）
- **安全装置:** /tmp/jarvis-nightly-stop, MAX_STEPS=10, MAX_RUNTIME=4h, コマンドブロックリスト
- **LaunchAgent:** com.jarvis.nightly (23:00) → jarvis-nightly.sh → nightly-forge-chrome.sh
- **Obsidian:** 90_System/NightlyForge/YYYY-MM-DD-forge.md
...
#### Access DB コンテキスト注入 (F3ポート)
- project-context-builder.sh に PowerShell クエリ追加
- プロジェクト名・販売先・納品先・開始日・最新見積を自動取得
- Parallels未起動時はスキップ（Dropbox+Obsidianのみで動作）

#### Claude Inbox ルーティング (F4ポート)
- orchestrator-chrome.ts: claudeInboxRoute()
- コード層でM番号・送信元マッチしない場合 → Inboxタブに投げてClaude判断
- JSON応答パース → confidence ≥ 0.6 で案件タブに転送
- Inboxタブ設定: getChromeOrchestrator().setInboxTab(wt, url)

#### SENDER_PROJECT_MAP 実データ (F4ポート)
- 6件: 伊藤ハム米久(M1317), 美山成田(M1317), 中西製作所(M1319), ヤガイ(M1311), 東海漬物(M1320), プリマハム(M1318)

#### Auto-Handoff (F7ポート)
- orchestrator-chrome.ts: checkAndHandoff()
- token-estimate 30+メッセージ → 要約生成 → 新チャット自動作成 → マッピング切替
- project-context-builder.sh でコンテキスト再注入

#### inject-file プリミティブ
- croppy-tab-manager.sh: inject-file <W:T> <filepath>
- ファイルパス渡し+base64エンコード → shell quoting問題を根本解決
- debate3/orchestrator/project-tab-routerが全て依存

## Domain Routing (専門チャットアーキテクチャ v1) (2026-03-16)
- **状態:** ✅ 稼働中 (Telegram実機テスト合格)
- **構成:**
  - autonomous/state/chat-routing.yaml — 15ドメイン定義 (keywords, URL, title_template, bootstrap, chat_history)
  - scripts/chat-router.py — route/url/set-url/get-field/archive-url/history
  - scripts/domain-relay.sh — 1タブnavigate方式リレー (タブ増殖禁止)
  - src/handlers/domain-router.ts — TSラッパー (quickDomainRoute + handleDomainRelay)
  - src/handlers/text.ts — ドメインルーティング優先 > Orchestrator M番号検出
- **ドメイン:** inbox, m1317, m1319, m1311, m1300, icad, access, fa, vision, pc, forge-code, forge-plc, forge-vision, forge-research, debate
- **rename:** Chrome JS + withCredentials方式 (sessionKey直接使用ではない)
- **Commits:** 0bd40a4〜26a1408 (v1構築), b152828 (inject修正), 399dc56 (優先度修正)

## Time Travel (過去チャット質問) (2026-03-16)
- **状態:** ✅ 実装完了
- **Script:** scripts/time-travel.sh
- **使い方:**
  - `./time-travel.sh <chat_id> "質問"` — chat_id直接指定
  - `./time-travel.sh --search "キーワード" "質問"` — search-chatlogs.pyで検索→質問
- **仕組み:** 元URLを保存 → 過去チャットにnavigate → inject → read-response → 元に戻る
- **用途:** 自律実装中に過去の設計決定・解決策を参照 (CLAUDE.mdにドキュメント化済み)
- **Commit:** ab7c569

## Domain Auto-Handoff (CONV_LIMIT自動世代交代) (2026-03-16)
- **状態:** ✅ 実装完了
- **Script:** scripts/domain-handoff.sh
- **トリガー:** domain-relay.sh が毎回relay後にtoken-estimate実行、≥70%でdomain-handoff.sh非同期発火
- **フロー:** 新チャット作成 → bootstrap注入 → chat-routing.yaml URL更新 → 旧IDをchat_history保存 → rename → Telegram通知
- **精度:** token-estimateはProject Instructions含まず過小評価 → 70%閾値で実質85-90%相当で発火（十分）
- **Commit:** ab7c569

## Handoff Auto-Rename (2026-03-16)
- **状態:** ✅ 実装完了
- **Script:** scripts/croppy-session-handoff.sh
- **仕組み:** SOURCE_WTを新タブ作成前に保存 → get-title → sed日付差し替え → rename-conversation
- **Commit:** 699069d

## Forge G14 Recon + Domain Routing (2026-03-16)
- **状態:** ✅ 実装完了
- **Recon (G14):** タスクキーワード抽出 → search-chatlogs.py直近30日 → 上位2件要約をプロンプト注入
- **Worker Tab Routing:** WORKER_WT固定廃止 → chat-routing.yamlでタスク種別→ドメイン→navigate
  - SKILL-PLC → forge-plc, SKILL-VISION → forge-vision, Research → forge-research, その他 → forge-code
- **relay tab共有:** domain-relay.shと同じタブ使用（増殖なし）
- **Commits:** 6e2a757 (recon), 7269d07 (routing)

## Domain Relay Buffer System (2026-03-24)
- **状態:** ✅ 稼働中
- **ファイル:** src/services/domain-buffer.ts, src/handlers/text.ts, scripts/domain-handoff.sh
- **機能:**
  - relay中のメッセージバッファリング（max 10件）
  - handoff中のメッセージバッファリング
  - relay完了後の自動バッファflush（まとめて1回inject）
  - lock自動期限（relay=5分、handoff=10分）
- **表示:** 📌 pc 応答中（バッファ N/10）/ 📌 pc HANDOFF中（バッファ N/10）
- **Handoff完璧引き継ぎ:**
  1. lock作成 → Telegram「📌 domain HANDOFF中」
  2. 旧🦞に要約inject → 旧🦞自身が全文脈で要約生成
  3. 新チャット作成 + bootstrap + 要約inject
  4. chat-routing.yaml URL切替
  5. バッファflush（まとめて新チャットにinject）
  6. lock削除 → Telegram「✅ domain HANDOFF完了」
- **サブコマンド:** --warm(事前作成) / --flush(バッファのみ) / --lock / --unlock

## Telegram UI Fixes (2026-03-24)
- 🗑削除ボタン: ib:del callback handler追加 + notify-dj.sh全通知に🗑ボタン付与
- トリアージ: line://→null, 空tel:→null, escalateにOK/取消ボタン
- ドメインリプライ: bot応答の📋[pc]/📌pcからドメイン逆引き→同ドメインリレー
- 全relay: execSync→async spawn（送信中→応答中→応答の3段遷移）

## Agent Bridge (read/execute mode) (2026-04-01)
- **Status:** DEPLOYED
- **Handler:** src/handlers/agent-task.ts
- **Endpoint:** POST localhost:3847/agent-task {prompt, mode: "read"|"execute"}
- **Bridge:** scripts/agent-bridge.sh <base64-prompt> <mode> <timeout>
- **read mode:** Read/Glob/Grep only, dontAsk, systemPrompt=read-only, maxTurns=10
- **execute mode:** Full tools, acceptEdits, maxTurns=15
- **SDK:** @anthropic-ai/claude-agent-sdk, Max Plan OAuth (no API key)
- **効果:** bootstrap 15kトークン→500 (30x削減)
- **Commits:** d12eb9e, b1ac6e2
