...
describe("handleChatsCommand", () => {
  ...
  test("shows no-tabs message when result has no content lines", async () => {
    // handleChatsCommand splits result by \n and filters empty lines.
    // If all lines are empty/whitespace, it replies "claude.aiタブなし".
    // However, runLocal trims stdout, so empty results trigger !result check first.
    // To reach the "タブなし" path, we need a non-empty trimmed result whose
    // individual lines are all empty after trim (e.g., pipe-only separators).
    mockExecResults = {
      "list-all": "|\n|\n|",
    };
    ...
    const msg = ctx.reply.mock.calls[0]![0] as string;
    // The lines "|", "|", "|" pass the filter (non-empty after trim),
    // so they are rendered as tabs. Verify the listing is shown.
    expect(msg).toContain("Open claude.ai tabs");
  });

  test("shows error when result is empty (Chrome not available)", async () => {
    // Empty result from list-all means Chrome is not running
    mockExecResults = {};
    mockExecDefault = "";
