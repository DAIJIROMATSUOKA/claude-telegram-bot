/**
 * /project command handler
 * Usage: /project M1314
 * Queries Access DB for project details via Parallels COM
 */

import type { Context } from "grammy";
import { isAuthorized } from "../security";
import { ALLOWED_USERS } from "../config";
import { exec } from "child_process";
import { promisify } from "util";

const execAsync = promisify(exec);
const LOOKUP_SCRIPT = `${process.env.HOME}/claude-telegram-bot/scripts/access-project-lookup.py`;

export async function handleProject(ctx: Context): Promise<void> {
  const userId = ctx.from?.id;
  if (!isAuthorized(userId, ALLOWED_USERS)) return;

  const text = ctx.message?.text || "";
  const arg = text.replace(/\/project\s*/i, "").trim();

  if (!arg) {
    await ctx.reply("使い方: /project M1314");
    return;
  }

  const mNum = arg.toUpperCase();
  if (!/^M\d{3,4}/.test(mNum)) {
    await ctx.reply("⚠️ M番号の形式で入力してください（例: M1314）");
    return;
  }

  const statusMsg = await ctx.reply(`🔍 ${mNum} の案件情報を検索中...`);

  try {
    const { stdout, stderr } = await execAsync(
      `python3 "${LOOKUP_SCRIPT}" "${mNum}"`,
      { timeout: 70_000 }
    );

    const result = stdout.trim();

    await ctx.api.editMessageText(
      ctx.chat!.id,
      statusMsg.message_id,
      result || `${mNum} の情報が見つかりませんでした。`
    );
  } catch (err: any) {
    const msg = err.stderr?.substring(0, 200) || err.message?.substring(0, 200) || "不明なエラー";
    await ctx.api.editMessageText(
      ctx.chat!.id,
      statusMsg.message_id,
      `❌ 検索エラー: ${msg}`
    );
  }
}
