...
async function main() {
  ...
  if (!botToken || !allowedUsers) {
    console.error('[EveningReview] Error: TELEGRAM_BOT_TOKEN and TELEGRAM_ALLOWED_USERS must be set');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('[EveningReview] Success');
    process.exit(0);
  } catch (error) {
    console.error('[EveningReview] Failed:', error);
    process.exit(1);
  }
}
