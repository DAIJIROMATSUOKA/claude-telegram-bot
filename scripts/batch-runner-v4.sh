...
#   batch-runner-v4.sh --parallel <prompt-directory>   # Agent Teams parallel
...
TIMEOUT_SEC=7200  # 120 minutes total for parallel mode
...
# --- Validate args ---
PROMPT_DIR="${1:?Usage: batch-runner-v4.sh [--parallel] <prompt-directory>}"
...
if [[ $PARALLEL -eq 1 ]]; then
  log "=== Batch Runner v4 PARALLEL started: $TOTAL tasks from $DIRNAME ==="
  bash "$NOTIFY" "🚀 Batch v4 PARALLEL started: $TOTAL tasks from $DIRNAME" 2>/dev/null || true

  # Build composite prompt
  COMPOSITE="You have $TOTAL independent tasks to execute in PARALLEL using agent teams.
Create an agent team. Each teammate handles exactly one task independently.
After ALL teammates finish, commit each task's changes separately with descriptive commit messages.
...
"
  ...
  for PROMPT_FILE in "${PROMPT_FILES[@]}"; do
    ...
    COMPOSITE+="--- Teammate $TASK_NUM: $BASENAME ---
$CONTENT
...
"
  ...
  COMPOSITE+="IMPORTANT:
- Each teammate works on its own task independently
- Do NOT wait for other teammates to finish before starting
- Each task gets its own git commit with a descriptive message
- Run tests after all tasks are complete
- Report a summary of all tasks when done"

  # Write composite prompt to temp file (avoid arg length limits)
  COMPOSITE_FILE="/tmp/batch-composite-prompt.txt"
  echo "$COMPOSITE" > "$COMPOSITE_FILE"
  ...
  # Count commits during this run
  COMMIT_COUNT=$(git -C "$REPO" log --oneline --since="$((TOTAL_DURATION + 120)) seconds ago" 2>/dev/null | wc -l | tr -d ' ')
  ...
  if [[ $EXIT_CODE -eq 0 ]]; then
    STATUS_ICON="✅"
    STATUS_TEXT="$TOTAL OK"
  elif [[ $EXIT_CODE -eq 124 ]]; then
    STATUS_ICON="⏱"
    STATUS_TEXT="TIMEOUT after ${TOTAL_MIN}min"
  else
    STATUS_ICON="❌"
    STATUS_TEXT="exit $EXIT_CODE"
  fi

  SUMMARY="🏁 Batch v4 PARALLEL complete
${STATUS_ICON} ${STATUS_TEXT} (${TOTAL} tasks)
📊 ${COMMIT_COUNT} commits | 🧪 Tests: ${TEST_STATUS}
⏱ Total: ${TOTAL_MIN}min (vs ~${TOTAL}x sequential)"
...
# =============================================
# SEQUENTIAL MODE (v3 compatible)
# =============================================
log "=== Batch Runner v4 started: $TOTAL tasks from $DIRNAME ==="
bash "$NOTIFY" "🚀 Batch v4 started: $TOTAL tasks from $DIRNAME" 2>/dev/null || true
...
for PROMPT_FILE in "${PROMPT_FILES[@]}"; do
  ...
  if [[ $EXIT_CODE -eq 124 ]]; then
    ...
    bash "$NOTIFY" "⏱ Batch timeout: $BASENAME" 2>/dev/null || true
