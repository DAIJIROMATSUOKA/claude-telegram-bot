...
tell application "Google Chrome"
  set t to tab $TIDX of window $WIDX
  set readJs to "(() => { var r = document.querySelectorAll('.font-claude-response'); if (r.length === 0) return 'NO_RESPONSE'; for (var idx = r.length - 1; idx >= 0; idx--) { var el = r[idx]; var clone = el.cloneNode(true); clone.querySelectorAll('details, summary, [class*=artifact]').forEach(function(n){n.remove();}); var txt = clone.innerText.trim(); if (txt.length >= 20) return txt.substring(0, 15000); var raw = el.innerText.trim(); if (raw.length >= 20) return raw.substring(0, 15000); } return 'NO_RESPONSE'; })()"
