...
tell application "Google Chrome"
  set t to tab $TIDX of window $WIDX
  set readJs to "(() => { var r = document.querySelectorAll('.font-claude-response'); if (r.length === 0) return 'NO_RESPONSE'; for (var idx = r.length - 1; idx >= 0; idx--) { var el = r[idx]; var md = el.querySelectorAll('p, ul, ol, h1, h2, h3, h4, pre, blockquote, table'); if (md.length === 0) { var inner = el.querySelector('div'); if (inner) md = inner.querySelectorAll('p, ul, ol, h1, h2, h3, h4, pre, blockquote, table'); } if (md.length > 0) { var parts = []; for (var j = 0; j < md.length; j++) { if (md[j].closest('details') || md[j].closest('summary')) continue; if (md[j].tagName !== 'TABLE' && md[j].closest('table')) continue; if (md[j].tagName === 'TABLE') { var rows = md[j].querySelectorAll('tr'); var tLines = []; for (var r = 0; r < rows.length; r++) { var cells = rows[r].querySelectorAll('th, td'); var cTexts = []; for (var c = 0; c < cells.length; c++) { cTexts.push(cells[c].innerText.trim()); } tLines.push(cTexts.join(' | ')); } parts.push(tLines.join(String.fromCharCode(10))); continue; } var t = md[j].innerText.trim(); if (t) parts.push(t); } var txt = parts.join(String.fromCharCode(10)); if (txt.length >= 5) return txt.substring(0, 15000); } var rawText = el.innerText.trim(); if (rawText.length >= 20) return rawText.substring(0, 15000); } return 'NO_RESPONSE'; })()"
...
check-status)
  ...
  # Use base64-encoded JS (detects TOOL_LIMIT + CONV_LIMIT + RATE_LIMIT)
  JS_B64=""
...
wait-response)
  ...
  while [ "$ELAPSED" -lt "$TIMEOUT_SEC" ]; do
    ...
    # CONV_LIMIT / RATE_LIMIT = unrecoverable, bail out immediately
    if [ "$STATUS" = "CONV_LIMIT" ] || [ "$STATUS" = "RATE_LIMIT" ]; then
      log "wait-response: $WT $STATUS detected — unrecoverable"
      echo "ERROR:$STATUS"
      exit 1
    fi
