#!/usr/bin/env python3
"""
session-bridge.ts を --resume 方式に書き換えるパッチ
Claude CLI のみ --resume 対応。Gemini/GPT は現行方式を維持。

使い方:
  python3 patch-session-bridge-resume.py
"""

import os

TARGET = os.path.expanduser("~/claude-telegram-bot/src/utils/session-bridge.ts")
...
print("1. AISession に cliSessionId?: string 追加")
print("2. Claude: --resume + --output-format json (履歴注入なし)")
print("3. Gemini/GPT: 従来方式を維持（履歴注入）")
print("4. parseClaudeJson() でセッションID + 応答テキスト抽出")
print("5. history[] は全AIで保持（/ai status用）")
print()
print("=== Next steps ===")
print("1. cd ~/claude-telegram-bot && bun build src/index.ts --outdir dist 2>&1 | head -5")
print("2. bun test 2>&1 | tail -30")
print("3. Jarvisを再起動して /ai claude でテスト")
print("4. 2回目のメッセージで会話が繋がるか確認")
